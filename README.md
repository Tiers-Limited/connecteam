...
- **Time clock IDs**: From step 2 (Time clocks). We use the first 25 clocks; each is used in the time-activities URL as `timeClockId`.
...
- **Job:** `data.job.title` (or `job.name`) → location for that job; we normalize it to one of our 4 location keys and attach it to each time entry.
