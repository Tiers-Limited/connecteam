...
import { FiMail, FiLock, FiLogIn, FiUsers, FiShield, FiEye, FiEyeOff, FiKey } from 'react-icons/fi';
...
export default function Login() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 dark:bg-slate-900 dark:text-slate-100 flex items-center justify-center p-4 sm:p-6 lg:p-8">
      <div className="w-full max-w-6xl grid lg:grid-cols-2 gap-6 lg:gap-8 items-stretch">
        {/* Left Panel - Branding (same indigo accent as Layout / app) */}
        <div className="hidden lg:flex flex-col rounded-2xl border border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-800/95 p-8 shadow-sm">
          <div className="flex items-center gap-3 mb-8">
            ...
            <span className="text-2xl font-semibold tracking-tight text-indigo-600 dark:text-indigo-400">ConnectTeam</span>
          </div>

          <div className="flex-1 flex flex-col justify-center space-y-6">
            <h2 className="text-2xl font-bold text-slate-800 dark:text-slate-100 leading-tight sm:text-3xl">
              Manage your team and operations in one place
            </h2>
            <p className="text-slate-600 dark:text-slate-400">
              Sign in with your admin account to access the dashboard, reports, and team tools.
            </p>
            <div className="flex items-start gap-3">
              <div className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-slate-200 dark:bg-slate-600">
        ...
        {/* Right Panel - Login Form (same surfaces as DailyTips) */}
        <div className="flex flex-col justify-center rounded-2xl border border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-800/95 shadow-sm p-8 lg:p-10">
          {/* Mobile Logo */}
          ...
          <div className="mb-8">
            ...
            <p className="mt-2 text-sm text-slate-600 dark:text-slate-400">
              ...
              {view === VIEW.FORGOT && 'Enter your email to receive a 6-digit PIN'}
              {view === VIEW.RESET && 'Enter the PIN from your email and your new password'}
            </p>
          </div>
          ...
          {view === VIEW.FORGOT && (
            <form onSubmit={handleRequestPin} className="space-y-6">
              ...
              <button
                ...
                className="w-full text-sm font-medium text-indigo-600 hover:text-indigo-500 dark:text-indigo-400"
              >
                Back to sign in
              </button>
            </form>
          ...
          {view === VIEW.RESET && (
            <form onSubmit={handleResetPassword} className="space-y-6">
              <div className="space-y-2">
                ...
                <div className="relative flex items-center rounded-lg border border-slate-200 bg-slate-50 py-2 pl-3 dark:border-slate-600 dark:bg-slate-800/50">
                  <span className="text-slate-600 dark:text-slate-400">{forgotEmail}</span>
                </div>
              </div>
              <div className="space-y-2">
                <label htmlFor="reset-pin" className="block text-sm font-medium text-slate-700 dark:text-slate-300">
                ...
                <div className="relative flex items-center rounded-lg border border-slate-300 bg-white shadow-sm transition-colors focus-within:ring-2 focus-within:ring-indigo-500 focus-within:border-indigo-500 dark:border-slate-600 dark:bg-slate-800 dark:focus-within:border-indigo-500">
                  ...
                  <input
                    ...
                    placeholder="000000"
                    disabled={loading}
                    className="w-full rounded-lg border-0 bg-transparent py-2 pl-10 pr-3 text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-0 dark:text-slate-100 dark:placeholder:text-slate-500 tracking-[0.4em] font-mono text-lg"
                  />
                </div>
              </div>
              <div className="space-y-2">
                ...
                <div className="relative flex items-center rounded-lg border border-slate-300 bg-white shadow-sm transition-colors focus-within:ring-2 focus-within:ring-indigo-500 focus-within:border-indigo-500 dark:border-slate-600 dark:bg-slate-800 dark:focus-within:border-indigo-500">
                  ...
                  <input
                    ...
                    disabled={loading}
                    className="w-full rounded-lg border-0 bg-transparent py-2 pl-10 pr-11 text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-0 dark:text-slate-100 dark:placeholder:text-slate-500"
                  />
                  <button
                    type="button"
              ...
              <button
                type="button"
                onClick={() => { setView(VIEW.LOGIN); setPin(''); setNewPassword(''); setConfirmPassword(''); }}
                className="w-full text-sm font-medium text-indigo-600 hover:text-indigo-500 dark:text-indigo-400"
              >
                Back to sign in
              </button>
          ...
          {/* Login form */}
          {view === VIEW.LOGIN && <form onSubmit={handleSubmit} className="space-y-6">
            <div className="space-y-2">
              <label
                htmlFor="email"
            ...
            <div className="flex items-center justify-end">
              <button
                type="button"
                onClick={() => { setView(VIEW.FORGOT); setForgotEmail(email); }}
